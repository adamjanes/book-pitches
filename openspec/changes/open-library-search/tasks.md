...
- [x] 1.3. Add `getBookCoverUrl(coverId: number | null, size: 'S' | 'M' | 'L')` helper that constructs `https://covers.openlibrary.org/b/id/{coverId}-{size}.jpg` or returns null if no coverId
...
- [x] 3.6. Handle errors: show "Search unavailable, please try again" on timeout (8s) or API failure
- [x] 3.7. Handle empty results: show "No books found" message
...
- [x] 4.3. On success, call an `onBookSelected(book: Book)` callback prop with the created/existing book record
- [x] 4.4. On error, display an error message without crashing
